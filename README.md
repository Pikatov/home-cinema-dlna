...
## Требования
- Go 1.26+.
- (Опционально) `ffprobe` из FFmpeg — нужно только для вычисления длительности и таймкода прогресса. Без `ffprobe` сервер продолжит работать, но таймкоды могут не отображаться.
